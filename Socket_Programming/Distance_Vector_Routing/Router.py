...
class Router:
    ...
    def InitializeNetworkVectors(length, nodesCommand):
        Router.networkVectors = [
            [float("inf") for x in range(length)] for y in range(length)]
        ...
        for i in range(Router.vectorList.__len__()):
            if i != Router.ID - 1:
                ...
                if Router.vectorList[i] == float("inf"):
                    ...
                    s = "Next hop is: " + \
                        Router.nextHopList[i] + " and the cost is: " + \
                        str(Router.vectorList[i])
                print("Shortest path: " + source +
                      "-" + destination + ": " + s)
        ...
        for i in range(Router.neighbours.__len__()):
            ...
            for j in range(Router.vectorList.__len__()):
                if j == Router.ID - 1:
                    ...
                    Router.vectorList[j] = Router.networkVectors[Router.ID -
                                                                 1][index] + Router.networkVectors[index][j]
    ...
    def ReadNetworkVectors():
        ...
        t3 = threading.Thread(
            target=Router.UpdateNetworkVectors, args=(message.split(":"), addr))
        t3.start()
